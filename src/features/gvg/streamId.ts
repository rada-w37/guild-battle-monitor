...
export interface GvgStreamScope {
...
export function buildGvgStreamId(scope: GvgStreamScope): GvgStreamId {
  ...
  return ((
    (scope.castleId << CASTLE_ID_SHIFT) |
...
export function createGuildBattleAllCastlesStreamScope(worldId: GvgWorldId | string): GvgStreamScope {
  const numericWorldId = Number(String(worldId).trim());

  if (!Number.isInteger(numericWorldId)) {
    throw new Error("worldId must be an integer");
  }
